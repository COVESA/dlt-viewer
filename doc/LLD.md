...
This document complements the HLD and gives more technical detail about the architecture, main modules, important data structures, interfaces, threading model, and message lifecycle. Use this LLD to guide implementation-level changes, tests, and plugin development.
...
This document complements the HLD and gives implementation-level detail for contributors: main modules, key classes, threading rules, data structures, and message lifecycle. It includes PlantUML snippets that live in `` and can be rendered to PNG/SVG for distribution.
...
Render the PlantUML sources under `` (e.g., `architecture.puml`, `lld_architecture.puml`). Sample snippets are shown below and are kept in the repo sources.

Component diagram (PlantUML source in `architecture.puml`)
