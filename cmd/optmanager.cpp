...
const QStringList &OptManager::getPostPluginCommands() const
{
...
{
#if (WIN32)
    qDebug()<<"Usage: dlt-cmd.exe [OPTIONS] [logfile] [projectfile] [filterfile] [mf4file] [pcapfile]";
#else
    qDebug()<<"Usage: dlt-cmd [OPTIONS] [logfile] [projectfile] [filterfile] [mf4file] [pcapfile]";
#endif

    qDebug()<<"UNDER WORK, not all options supported yet!";
    qDebug()<<"Options:";
    ...
    qDebug()<<"Examples:";
    #if (WIN32)
    qDebug()<<"  dlt-cmd.exe -c c:\\trace\\trace.dlt .\\trace.txt";
    qDebug()<<"  dlt-cmd.exe -s -c -u c:\\trace\\trace.dlt .\\trace.txt";
    qDebug()<<"  dlt-cmd.exe -s -d -c c:\\trace\\trace.dlt .\\trace.dlt";
    qDebug()<<"  dlt-cmd.exe -s -p \\proj\\decodeded.dlp -dd -c c:\\trace\\trace.dlt .\\trace.dlt";
    qDebug()<<"  dlt-cmd.exe -s -csv -c c:\\trace\\trace.dlt .\\trace.csv";
    qDebug()<<"  dlt-cmd.exe -s -d -f c:\\filter\\filter.dlf -c c:\\trace\\trace.dlt .\\filteredtrace.dlt";
    qDebug()<<"  dlt-cmd.exe -p c:\\proj\\export.dlp -l c:\\trace\\trace.dlt -e \"Filetransfer Plugin|export|ftransferdir\"";
    qDebug()<<"  dlt-cmd.exe trace_1.dlt trace_2.dlt";
    #else
    qDebug()<<"  dlt-cmd -c ./traces/trace.dlt ./trace.txt";
    qDebug()<<"  dlt-cmd -s -c -u ./trace/trace.dlt ./trace.txt";
    qDebug()<<"  dlt-cmd -s -d -c ./trace/trace.dlt ./trace.dlt";
    qDebug()<<"  dlt-cmd -s -p ./proj/decodeded.dlp -dd -c ./trace/trace.dlt ./trace.dlt";
    qDebug()<<"  dlt-cmd -s -csv -c ./trace/trace.dlt ./trace.csv";
    qDebug()<<"  dlt-cmd -s -d -f ./filter/filter.dlf -c ./trace/trace.dlt ./filteredtrace.dlt";
    qDebug()<<"  dlt-cmd -p ./proj/export.dlp -l ./trace/trace.dlt -e \"Filetransfer Plugin|export|./ftransferdir\"";
    qDebug()<<"  dlt-cmd trace_1.dlt trace_2.dlt";
    #endif
}
...
QString OptManager::getWorkingDirectory() const { return workingDirectory; }
