...
TEST(CtrlPayload, parse) {
    ...
    auto payload = qdlt::msg::payload::parse({data.constData(), (unsigned long)data.size()}, false);
    ...
    EXPECT_EQ(qdlt::msg::payload::asString(app.id), "SYS");
    EXPECT_EQ(app.description, "DLT System Manager");
    ASSERT_EQ(app.ctxs.size(), 1);

    auto ctx = app.ctxs[0];
    EXPECT_EQ(qdlt::msg::payload::asString(ctx.id), "MGR");
    EXPECT_EQ(ctx.logLevel, 0xff);
    EXPECT_EQ(ctx.traceStatus, 0xff);
