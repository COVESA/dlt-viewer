...
public:
    ...
    */
    ...
    //! Get Index of all DLT messages matching the active filter PLUS a set of manually marked indices.
    /*! The returned vector preserves the base-filter ordering and inserts missing marked indices
     *  according to the current sort mode.
     */
    QVector<qint64> getIndexFilterBasePlusManualMarkers(const QList<unsigned long int> &indices) const;
